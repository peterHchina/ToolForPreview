...
- (IBAction)sendMessage:(id)sender {
  if (connectedChannel_) {
    ...
    dispatch_data_t payload = PTExampleTextDispatchDataWithString(@"13234234" ,dataFromImage);
    ...
    [self presentMessage:[NSString stringWithFormat:@"[you]: %@", @""] isStatus:NO];
...
-(void) setUpStatusBar{
    ...
    RHStatusItemView* statusView = [[RHStatusItemView alloc] initWithStatusBarItem:statusItem];
...
- (void)ioFrameChannel:(PTChannel*)channel didReceiveFrameOfType:(uint32_t)type tag:(uint32_t)tag payload:(PTData*)payload {
  ...
  if (type == PTExampleFrameTypeDeviceInfo) {
    ...
    NSString *message = [[NSString alloc] initWithBytes:textFrame->utf8text length:textFrame->length encoding:NSUTF8StringEncoding];
      ...
      textFrame->imageLength = ntohl(textFrame->imageLength);
      NSString *imageString = [[NSString alloc] initWithBytes:textFrame->MByte length:textFrame->imageLength encoding:NSUTF8StringEncoding];
